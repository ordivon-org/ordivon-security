...
class SecurityResearchOwnerPublicationTests(unittest.TestCase):
    ...
    def test_manifest_digest_and_79_anchors_revalidate(self):
        ...
        self.assertEqual(self.manifest["inventory"]["canonicalDocs"], 75)
        self.assertEqual(len(self.manifest["anchors"]), 79)
    ...
    def test_projection_is_bounded_not_second_authority_map(self):
        ...
        self.assertEqual(len(result_refs), 24)
    ...
    def test_negative_and_historical_standing_are_not_promoted(self):
        ...
        self.assertIn(("result:security:ace4-capability-consequence-misrepresentation-established", "STANDING:ACCEPTED"), standing)
        self.assertIn(("result:security:ace10-deterministic-consequence-reduction-accepted", "STANDING:ACCEPTED"), standing)
        self.assertIn(("result:security:ace11-current-effect-binding-same-surface-admission-accepted", "STANDING:ACCEPTED"), standing)
        self.assertIn(("result:security:p1-current-capability-surface-transfer-accepted", "STANDING:ACCEPTED"), standing)
