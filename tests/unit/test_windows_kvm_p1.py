...
class WindowsKvmP1Tests(unittest.TestCase):
    ...
    def test_public_davinci_media_index_does_not_authorize_execution(self) -> None:
        path = (
    ...
    def test_davinci_static_decision_rejects_execution_profile(self) -> None:
        path = (
            Path(__file__).parents[2]
            / "research"
            / "cases"
            / "windows-kvm-p1-davinci-static-entry.json"
        )
    ...
    def test_davinci_causality_reassessment_keeps_unproven_edges_explicit(self) -> None:
        path = (
            Path(__file__).parents[2]
            / "evidence"
            / "acceptance"
            / "windows-kvm-p1-davinci-causality-r2.json"
        )
        value = __import__("json").loads(path.read_text(encoding="utf-8"))
    ...
    def test_davinci_research_admission_does_not_reverse_product_rejection(self) -> None:
        path = (
            Path(__file__).parents[2]
            / "research"
            / "cases"
            / "windows-kvm-p1-davinci-isolated-research-trial.json"
        )
        value = __import__("json").loads(path.read_text(encoding="utf-8"))
        self.assertEqual(
            value["kind"],
