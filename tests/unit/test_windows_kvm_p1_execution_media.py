...
class WindowsKvmP1ExecutionMediaTests(unittest.TestCase):
    ...
    def test_retained_case_a_contract_keeps_execution_closed(self) -> None:
        ...
        case_value = json.loads(
            (
                repo
                / "research/cases/windows-kvm-p1-davinci-case-a-original-repack.json"
            ).read_text(encoding="utf-8")
        )
        transform_value = json.loads(
            (
                repo / "research/cases/windows-kvm-p1-davinci-case-a-transform.json"
            ).read_text(encoding="utf-8")
        )
        contract_value = json.loads(
            (
                repo / "research/cases/windows-kvm-p1-davinci-case-a-execution.json"
            ).read_text(encoding="utf-8")
        )
        ...
        self.assertEqual(
            retained_contract.installer_relative_path, "DaVinci Resolve Studio.exe"
        )
