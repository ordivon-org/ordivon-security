...
The revised first S5 falsifier is intentionally smaller than a multi-VM Range: one disposable Windows Guest must join an externally owned isolated L2 fabric with one lightweight synthetic peer. The Guest may reach that peer, but it must have no route to the Host management plane or external network. Host-owned topology/lifecycle facts, external packet observation, QMP control, and complete fabric/machine residual closure must survive without trusting the Guest. Only after this boundary is physically accepted should a second full VM or additional lightweight nodes be introduced.
