...
C1-G therefore corrects the interpretation of claim CAS: **ledger digest is a durable publication/identity fence, not a physical-world progress version**. Recovery authority acquisition must be followed by current world observation before choosing adopt/continue/repair/close. This still did not force durable substep state, a transaction log, a generation counter, or a causal DAG. The strongest next pressure is unpublished completion: allow peer B and its one-shot consequence to be consumed before stable publication, kill the successor, and test whether persistent topology plus independent consequence evidence can distinguish completed-but-unpublished from still-partial without replay.
