...
C1-M closes the first compensation-information-loss pressure at physical revision `404e7e691fd2ed6e557ad525c9ff0b63c8aceedd`. Downstream repair truth remains durable but private: UID 65534 can invoke compensation but cannot read `balance.json`. Under a naive subtract-one compensation identity, compensated and uncompensated histories have byte-equivalent sender ledgers (`sha256:c0f7551b7a6f9ac1c70af9f8cbc64fddaed1aca21bda3469e93743c526dfc38e`) and caller views (`sha256:485509bf26a8527f77c59edd06a37b0241f96ec39f4c2ce259ebddf9a6285704`) despite private truth 1 versus 2; caller classification is therefore `UNKNOWN`, and blind retry produces 0 versus 1. A distinctly identified `private-counter.ensure-repaired` protocol then preserves the same caller uncertainty within its two histories while safely returning `already-repaired` for hidden 1 and `applied` for hidden 2; both converge to 1. No sidecar receipt/dedup object exists: every private world inventory is exactly `[balance.json]`. The next pressure is downstream truth loss/corruption/fork, not another caller-side receipt.
