...
C1-D therefore gives real evidence that **world state can act as progress state** for this exact effect. It also exposes the next structural pressure: during continuation the durable owner still points to the dead original controller, so a concurrently invoked reconciler would still classify the world as orphaned. The next experiment should race a successor continuation controller against reconciliation and let that pressure determine the minimum durable claim/lease/CAS or ownership-transfer mechanism. Do not add a global recovery lock or generic workflow first.
