...
This single-effect consumer did **not** force a generic `RangeEvent.causalParents` DAG or durable `RangeSession`: exact durable effect identity + durable physical phase + independent Host truth was enough to classify the tested interruptions. The next stronger pressure is partial materialization *inside* peer-B creation, before a stable `peer-b-present` phase exists. Interrupt namespace/veth/service construction and require a replacement process to decide whether to finish the suffix, repair, compensate, or close to zero. Let that experiment determine whether finer durable substep identity, idempotent suffix continuation, compensation semantics, or stronger causal structure are actually unavoidable.
