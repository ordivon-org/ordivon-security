...
The next pressure can now come from a genuinely different consumer. A strong candidate is the first consequential world change deliberately requested by a contested Actor: preserve proposal/admission/execution/effect-verification separation and determine whether a persistent Range action gateway becomes unavoidable. Audit that path against current code before introducing any generic action or topology framework.
