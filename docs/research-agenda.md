...
The next pressure should add only one new independent variable: for example, another lightweight peer/service or a topology change during one live Range. The question is whether current backend-local node/fabric identity plus external truth remains sufficient. A second full VM should be introduced only when the hypothesis genuinely requires another independent OS/kernel/failure domain; a reusable node/topology/materialization abstraction should appear only after repeated real consumers expose the same mechanism.
