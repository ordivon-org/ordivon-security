...
C1-J closes that recipient commit-gap pressure at physical revision `6563613bc0757f8db15caf1c9014bab577d893f8`. `effect → SIGKILL → marker` produced one pre-crash pulse, no marker, and two total pulses after retry. Reversing the order to `marker → SIGKILL → effect` produced a marker, zero pulses, and retry suppression, permanently losing the consequence. A second contrast used an honest durable inbox phase `reserved`: crash-before-effect and effect-then-crash-before-completed left the same inbox digest `sha256:107ecb3e4c64ad93a1c8028467f5bf241c7f0c243b6b2c19b55806f1ee56a8d9` and the same recovery-view digest `sha256:8be64a988cfa78d73c18aaca93e549aa7726f5867c26f82d601671945be38107`, despite ground truth of zero versus one pulse. Retry yields one versus two pulses; suppression yields zero versus one. C1-J therefore proves ordering is not atomicity and reservation can preserve `UNKNOWN` without resolving it. The next smallest pressure is intrinsic downstream idempotency before any generic transaction framework.
