...
The next sequence remains provider-first rather than gateway-first: CA3 now pressures post-compromise control/authority/continuity/reachability and delegated/self-propagating continuation; CA4 pressures classical Blue observation/detection/adjudication/response. Only repeated binding friction across those materially different consumers may admit a shared Security provider seam in CA5. Existing `RangeEffectRequest`, Harness Tool composition, Runtime execution, and provider-specific adapters remain the simpler baseline.
