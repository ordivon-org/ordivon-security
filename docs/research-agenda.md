...
Partial materialization therefore forced **transient resource-ownership completeness**, not a generic `RangeEvent.causalParents` DAG, durable `RangeSession`, or transaction log. The next pressure is no longer cleanup: start a fresh controller from the accepted partial world and attempt an idempotent peer-B suffix while keeping the Windows Guest alive. Add finer durable substep identity, repair/compensation semantics, or stronger causal ordering only if exact effect identity + exact resource identity + Host observation cannot make that continuation safe.
