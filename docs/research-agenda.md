...
The durable law is **one recovery mutator per exact world generation**, not “use flock everywhere.” C1-E did not force a TTL, heartbeat daemon, distributed lease, transaction framework, or causal DAG in the current single-host universe. The nearest next pressure is two successor candidates competing for the same generation, including whether a loser can later inherit after the winner dies. Multi-node fencing/consensus should wait until a real multi-node world exists.
