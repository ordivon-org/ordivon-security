...
The next sequence remains provider-first rather than gateway-first: CA2 now pressures vulnerability discovery/exploitability providers; CA3 pressures post-compromise control/authority/continuity/reachability and propagation; and CA4 pressures classical Blue observation/detection/adjudication/response. Only repeated binding friction across those materially different consumers may admit a shared Security provider seam in CA5. Existing `RangeEffectRequest`, Harness Tool composition, Runtime execution, and provider-specific adapters remain the simpler baseline.
