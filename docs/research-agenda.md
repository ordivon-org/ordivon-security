...
S0 is only a contract/lifecycle foundation. S1 confirms that the accepted synchronous Contest core composes naturally as a bounded profile through RangeSession management events and sealed evidence references; forcing `ContestRunner` to implement `RangeSessionBackend` is unnecessary. S2 separates reusable Windows KVM machine authority from Evaluation Sample admission: disposable overlay/UEFI/TPM state, root-owned ledger, QMP topology truth, process identity, recovery, and residual closure now exist below the Evaluation adapter. S3 then physically exercises one sacrificial Windows node whose maintained canary kills Guest-side observation/bootstrap assumptions, installs synthetic reboot persistence, deletes a synthetic Guest log, and reboots. The accepted management claim comes only from external QMP/process/ledger/closure facts: no network-class device, Guest-originated reset observed, QEMU lifecycle retained, and zero residual closure. Guest claims remain contested. The next falsifier is S4: determine which important Guest/world changes can be reconstructed when Guest sensors lie or disappear, without turning the Guest Observer back into authority.
