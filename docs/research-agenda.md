...
C1-F therefore strengthens the durable rule to include **recovery lineage**, while still not forcing election priority, fairness, a generation counter, a wall-clock lease, distributed consensus, transaction logging, or a causal DAG. The strongest next pressure is not a third stable successor merely to lengthen the chain: kill a winning successor midway through its own continuation, after physical progress but before stable generation publication, and require another successor to infer the remaining suffix from current world truth plus lineage.
