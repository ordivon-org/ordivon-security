...
C1-A remains the canonical physical proof that Range authority is optional strategic power rather than an action mandate, that model intent is distinct from Security admission/execution, and that retained Range observations must be immutable snapshots. The reusable semantics forced by this experiment have since graduated into the current AF2 `RangeEffectInterface`, `RangeIntentContext`, `RangeIntentDecision`, and Harness integration, with broader current unit coverage in `test_autonomous_range_intent.py`.

The original one-shot `cli_windows_kvm_c1a_acceptance.py` physical runner is therefore retained under `fixtures/archive/runners/` rather than the current package. Its corrected accepted apparatus is source-fenced at `f692c22492e5b998df8373bcd165001e059307cf`; the earlier evidence-aliasing falsifier remains part of this document's history. Restoring the runner is an explicit physical reproduction action, not a prerequisite for current AF2 capability.
