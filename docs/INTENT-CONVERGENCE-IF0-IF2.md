...
The same audit separated IF0's historical finalization treatment from the exact AC2 mismatch fixture reused by IF2 and IF3. That fixture now lives in `intent_convergence_research_fixture.py`, while IF0's one-shot `cli_intent_finalization_if0_acceptance.py` runner is archived with apparatus revision `4e30b93a71d0132522234e05cc7bf93cc5af9a57`. The current finalized-intent integration remains independently tested and is still consumed by IF2; archiving IF0 therefore removes historical orchestration, not the current integration contract or the IF0 authority-boundary result.
