...
C1-B's interrupted-consequence standing, durable effect-binding correction, world-reobservation law, S6 recovery mechanics and exact acceptance evidence remain current. The one-shot `cli_windows_kvm_c1b_acceptance.py` fault-orchestration module was retired from HEAD after the namespace/process/semantic-binding/machine-config helpers used by later C1-C/D experiments moved into narrow Windows-KVM recovery-acceptance support. The accepted C1-B apparatus remains recoverable from its exact `3606734` test revision and earlier implementation revisions recorded in the acceptance receipt. Keeping those evidence laws does not require keeping the original SIGKILL orchestration module permanently executable.
