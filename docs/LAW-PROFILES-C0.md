...
| `RangeEvent.causalParents` | **keep intent; C1-B through C1-K resolve interruption, information loss, a recipient commit-gap, and one idempotent consequence without a generic DAG; causal structure cannot make independent commits atomic, and C1-K shows some declarative effects do not require exact invocation history for semantic satisfaction, so strengthen only for a concrete shared-effect/compensation/distributed-causation consumer** |
