...
| retry safety is not proof of non-delivery | **keep as effect-protocol law; C1-I shows blind resend duplicates a non-idempotent delivered consequence, while exact effect identity plus recipient-owned durable duplicate suppression makes same-effect retry safe without revealing recipient-private history** |
...
| `RangeEvent.causalParents` | **keep intent; C1-B through C1-I resolved interruption, partial materialization, continuation, arbitration, lineage, completed-but-unpublished recovery, and one genuine information-loss case without a generic DAG; C1-I shows a graph cannot recreate facts that no longer exist, so strengthen causal structure only when shared effects, compensation, or distributed causation has an actual consumer** |
