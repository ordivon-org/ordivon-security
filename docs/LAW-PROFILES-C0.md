...
| `RangeEvent.causalParents` | **keep intent; C1-B through C1-H resolved interruption, partial materialization, continuation, arbitration, lineage, mid-successor loss, and one completed-but-unpublished consequence without a generic DAG, so strengthen only when information loss, shared effects, compensation, or distributed causation creates genuine ambiguity** |
