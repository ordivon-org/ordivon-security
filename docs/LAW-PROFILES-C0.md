...
| `RangeEvent.causalParents` | **keep intent; C1-B through C1-M now cover interruption, information loss, commit gaps, intrinsic idempotency, observable compensation, and private-truth compensation without a generic DAG; explicit original/compensation identity remains sufficient for these consumers, so strengthen only when shared or distributed causal structure creates a concrete ambiguity** |
