...
| `RangeEvent.causalParents` | **keep intent; C1-B through C1-J resolve interruption, continuation, information loss, and a recipient commit-gap without a generic DAG; C1-I/J show that adding causal structure cannot make two independent commits atomic or recreate a vanished fact, so strengthen only for a concrete shared-effect/compensation/distributed-causation consumer** |
