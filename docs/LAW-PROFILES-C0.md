...
| `RangeEvent.causalParents` | **keep intent; C1-B through C1-L now cover interruption, information loss, recipient commit gaps, intrinsic idempotency, and one compensable duplicate without a generic DAG; C1-L keeps original and compensation identities distinct but still does not force a generic causal graph because the repair relation has one explicit consumer** |
