...
- [`ADVERSARIAL-CAPABILITY-ENVIRONMENT-ACE0.md`](ADVERSARIAL-CAPABILITY-ENVIRONMENT-ACE0.md) owns the ACE0 direct Tool-output/effect-description poisoning falsifier, its fixed-authority/fixed-world no-effect boundary, and the negative result against promoting a prompt-injection guard or new Security policy layer from this consumer.
- [`ADVERSARIAL-CAPABILITY-ENVIRONMENT-ACE1-11.md`](ADVERSARIAL-CAPABILITY-ENVIRONMENT-ACE1-11.md) owns the ACE1–ACE11 scaffold-ablation, capability-consequence misrepresentation, failed naive repairs, deterministic consequence reduction, exact effect currentness and same-current-surface returned-intent admission results.
...
- [`P1-CAPABILITY-SURFACE-TRANSFER.md`](P1-CAPABILITY-SURFACE-TRANSFER.md) owns the accepted real-provider current-capability-surface transfer, UNKNOWN/UNAVAILABLE fault-injection falsifier, same-surface pre-provider rejection boundary, model-control non-result, and rejection of hidden-truth compilation or a global capability compiler.
...
- [`ORDINARY-SECURITY-CAPABILITY-PREFLIGHT-R2.md`](ORDINARY-SECURITY-CAPABILITY-PREFLIGHT-R2.md) owns the owner-local read-only mechanical eligibility projection, exact record-selection standing transition, model-facing Tool-withdrawal boundary, and rejection of a new Security planner/registry/compiler.
