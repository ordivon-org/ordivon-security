...
### R6-B: layered sealed P1 base

Reinstalling the full Windows base for every small Controller/resource revision was tested as an avoidable trust and cost expansion. A benign offline canary first established a narrower substrate: a qcow2 child layer backed by the already accepted Windows base could be attached read-write through Host-only qemu-nbd/NTFS, modified only under `C:\ProgramData\Ordivon`, disconnected, attached again read-only, and independently read back while the parent base digest remained unchanged. The canary also exposed two operating facts that now belong to the sealing contract: WSL may require `modprobe nbd max_part=16` after restart, and `partx -d` is only an auxiliary cleanup operation; authoritative closure is qemu-nbd disconnect plus the NBD block size returning to zero.

The first derived-base implementation at `c4f2958` exposed a separate permission regression. It changed the shared provider `images/` directory to mode `0700`; the derived qcow2 itself remained `root:qemu 0440`, but the `qemu` run user could no longer traverse the directory, so QEMU failed before QMP readiness with a backing-image permission error. Residual closure remained clean. The implementation was corrected at `5d4b9ca` to preserve the provider's established `root:qemu 0710` images-directory authority, with a regression test enforcing that boundary.

From exact clean revision `5d4b9ca`, the sealer produced a one-megabyte qcow2 child whose environment identity is `sha256:1de50319824bce16d0442382c01a2623acc0b3309fcde51aa50dad938347f332`. The child explicitly binds its accepted parent manifest/environment/base-image identities, its two-entry backing chain, the exact Vault resources copied into fixed Ordivon paths, the sealing-tool identities, read-only post-write verification, parent-image immutability, and both read-write/read-only NBD closure. The exact sealed resources are the accepted generic Controller `sha256:eb7e9874f1dc568721c826ea30e1b77f325254244564ca70381d2556f3d4388a` at `C:\ProgramData\Ordivon\p1-controller.exe` and the accepted R5 execution-control canary `sha256:d29becd1409bab42bbba885b3e6db5623cedaf61d83d6c3b01ed7111e347d655` under the Ordivon acceptance subtree. No third-party execution authority is added by the manifest.

A separate maintained probe committed at `5eaf2da` then booted that exact derived environment through the normal Windows KVM Provider. The probe has no arbitrary target argument; it hashes the two fixed sealed paths in the Guest, requires the exact accepted byte lengths/digests, invokes only the Controller's accepted `--self-test` and the execution-control canary's accepted `--result` self-test, and validates their semantic result markers. Both sealed identities matched, both self-tests exited zero, the Controller's manifest/timeout ownership checks passed, and the R5 canary again proved writable root/nested staging with denied PE execution. QMP reported zero network devices, QEMU exited zero, the Provider reported no error, and residual closure was clean. The sanitized acceptance index is [`../evidence/acceptance/windows-kvm-p1-derived-base-5d4b9ca.json`](../evidence/acceptance/windows-kvm-p1-derived-base-5d4b9ca.json).
