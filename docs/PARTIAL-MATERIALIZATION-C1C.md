...
C1-C's partial-materialization result, exact transient-resource ownership correction, independent Host observation requirement, reconciler behavior, S6 normal-path regression and source-fenced acceptance evidence remain current. The reusable fault world used by C1-D now lives in `windows_kvm_partial_world_fixture.py`: it owns the accepted deterministic resource names, root-link observation and the exact kill-after-root-veth fixture. The original one-shot `cli_windows_kvm_partial_materialization_acceptance.py` orchestration was retired from HEAD because no current command or consumer required the complete historical runner once that fixture responsibility was separated. The accepted fixed trial remains exactly recoverable from revision `39693ebf1fc51a2814a8d15ddc136530ecf46533`; the earlier false-clean baseline remains recoverable from `2458ad7225d3e4727f06534c2ebc7f12409bca88`.
