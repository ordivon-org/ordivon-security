...
_FIXTURE_ID = "ordivon-p1-sealed-base-probe-v1"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Execute a maintained probe that verifies and self-tests only the exact sealed P1 "
            "Controller and execution-control resources inside a derived Windows base."
        )
    )
...
def _compile_sealed_base_probe(output_path: Path) -> JsonObject:
    ...
    for path in (source_path, compiler, objdump):
        if path.is_symlink() or not path.is_file():
            raise ValueError(f"P1 sealed-base probe dependency is missing or unsafe: {path}")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.parent.chmod(0o700)
    if output_path.exists():
        raise FileExistsError(f"P1 sealed-base probe output already exists: {output_path}")
    command = [
        str(compiler),
        "-municode",
        "-Os",
    ...
    if matches:
        ...
        raise ValueError(f"P1 sealed-base probe imports prohibited network APIs: {matches}")
    ...
    return {
        ...
        "fixedSealedTargets": {
            "genericController": {
                "path": "C:\\ProgramData\\Ordivon\\p1-controller.exe",
                "digest": "sha256:eb7e9874f1dc568721c826ea30e1b77f325254244564ca70381d2556f3d4388a",
                "byteLength": 25_600,
            },
            "executionControlCanary": {
                "path": "C:\\ProgramData\\Ordivon\\acceptance\\p1-execution-control-canary.exe",
                "digest": "sha256:d29becd1409bab42bbba885b3e6db5623cedaf61d83d6c3b01ed7111e347d655",
                "byteLength": 27_648,
            },
        },
...
def main() -> None:
    ...
    compilation = _compile_sealed_base_probe(fixture_path)
    try:
        ...
        guardian = GuardianPolicy(
            policy_id="guardian-policy:windows-kvm-p1-sealed-base-probe",
            revision="1",
        ...
        observation = ObservationPlan(
            plan_id="observation-plan:windows-kvm-p1-sealed-base-probe",
            revision="1",
        ...
        environment = EnvironmentIdentity(
            environment_id="environment:windows-kvm-p1-sealed-base-probe",
        ...
        authority = AuthorityManifest(
            authority_id="authority:ordivon-p1-sealed-base-probe",
            revision="1",
            sample_digest=sample.sha256,
            operator_id="operator:local",
            authorization_basis=(
                "Ordivon-maintained probe that verifies exact sealed resource bytes and runs "
                "only their accepted self-test interfaces in a disposable no-network Guest."
            ),
        ...
        spec = EvaluationSpec(
            evaluation_id="evaluation:windows-kvm-p1-sealed-base-probe",
            revision="1",
            sample=sample,
            authority=authority,
