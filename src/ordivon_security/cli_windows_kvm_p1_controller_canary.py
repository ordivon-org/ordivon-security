...
_FIXTURE_ID = "ordivon-p1-controller-canary-v1"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compile and execute the Ordivon-maintained P1 Controller canary in one "
            "disposable deny-all Windows KVM Evaluation Run. No third-party Sample is used."
        )
    )
...
def _compile_controller_canary(output_path: Path) -> JsonObject:
    source_resource = files("ordivon_security").joinpath(
        "resources", "windows_kvm", "p1_controller_canary.c"
    )
    ...
    for path in (source_path, compiler, objdump):
        if path.is_symlink() or not path.is_file():
            raise ValueError(f"P1 Controller canary dependency is missing or unsafe: {path}")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.parent.chmod(0o700)
    if output_path.exists():
        raise FileExistsError(f"P1 Controller canary output already exists: {output_path}")
    command = [
        str(compiler),
        "-municode",
        "-Os",
    ...
    if matches:
        ...
        raise ValueError(f"P1 Controller canary imports prohibited network APIs: {matches}")
    ...
    return {
        ...
        "canaryClaims": [
            "job-object-descendant-ownership",
            "kill-on-job-close",
            "active-process-limit-secondary-block",
        ],
        "selectiveSecondaryBlocking": False,
    }
...
def main() -> None:
    ...
    compilation = _compile_controller_canary(fixture_path)
    try:
        ...
        guardian = GuardianPolicy(
            policy_id="guardian-policy:windows-kvm-p1-controller-canary",
            revision="1",
        ...
        observation = ObservationPlan(
            plan_id="observation-plan:windows-kvm-p1-controller-canary",
            revision="1",
        ...
        environment = EnvironmentIdentity(
            environment_id="environment:windows-kvm-p1-controller-canary",
        ...
        authority = AuthorityManifest(
            authority_id="authority:ordivon-p1-controller-canary",
            revision="1",
            sample_digest=sample.sha256,
            operator_id="operator:local",
            authorization_basis=(
                "Ordivon-maintained Controller canary compiled locally to test Windows Job "
                "Object process-tree ownership and a deliberately broad secondary-process "
                "limit inside a disposable no-network Windows KVM Guest."
            ),
        ...
        spec = EvaluationSpec(
            evaluation_id="evaluation:windows-kvm-p1-controller-canary",
            revision="1",
            sample=sample,
            authority=authority,
