...
_FIXTURE_ID = "ordivon-p1-execution-control-canary-v1"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compile and execute the Ordivon-maintained P1 selective execution-control "
            "canary in one disposable deny-all Windows KVM Evaluation Run."
        )
    )
...
def _write_header(script: bytes, path: Path) -> None:
    values = ",".join(f"0x{value:02x}" for value in script)
    path.write_text(
        "#ifndef ORDIVON_P1_EXECUTION_CONTROL_SCRIPT_H\n"
        "#define ORDIVON_P1_EXECUTION_CONTROL_SCRIPT_H\n"
        f"static const unsigned char embedded_script[] = {{{values}}};\n"
        f"static const unsigned int embedded_script_len = {len(script)}u;\n"
        "#endif\n",
        encoding="ascii",
    )


def _compile_execution_control_canary(output_path: Path) -> JsonObject:
    launcher = _resource("p1_execution_control_launcher.c")
    script = _resource("p1_execution_control_canary.ps1")
    compiler = Path("/usr/bin/x86_64-w64-mingw32-gcc")
    objdump = Path("/usr/bin/x86_64-w64-mingw32-objdump")
    for path in (launcher, script, compiler, objdump):
        if path.is_symlink() or not path.is_file():
            raise ValueError(f"P1 execution-control canary dependency is missing or unsafe: {path}")

    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.parent.chmod(0o700)
    if output_path.exists():
        raise FileExistsError(f"P1 execution-control canary output already exists: {output_path}")

    with tempfile.TemporaryDirectory(prefix="ordivon-p1-policy-build-") as temp_name:
        temp = Path(temp_name)
        header = temp / "p1_execution_control_script.h"
        _write_header(script.read_bytes(), header)
        command = [
            str(compiler),
            "-municode",
            "-Os",
    ...
    if matches:
        ...
        raise ValueError(f"P1 execution-control canary imports prohibited network APIs: {matches}")
    ...
    return {
        ...
        "launcherSourceDigest": _digest(launcher),
        "policyScriptDigest": _digest(script),
        "fixtureDigest": _digest(output_path),
        ...
        "targetStagingSurface": "%TEMP%\\DaVinci Resolve\\**\\PE",
        "canaryClaims": [
            "ntfs-inherited-execute-deny",
            "staging-surface-remains-writable",
            "allowed-descendant-executes",
            "staging-path-pe-denied",
            "nested-staging-path-pe-denied",
        ],
    }
...
def main() -> None:
    ...
    compilation = _compile_execution_control_canary(fixture_path)
    try:
        ...
        guardian = GuardianPolicy(
            policy_id="guardian-policy:windows-kvm-p1-execution-control-canary",
            revision="1",
        ...
        observation = ObservationPlan(
            plan_id="observation-plan:windows-kvm-p1-execution-control-canary",
            revision="1",
        ...
        environment = EnvironmentIdentity(
            environment_id="environment:windows-kvm-p1-execution-control-canary",
        ...
        authority = AuthorityManifest(
            authority_id="authority:ordivon-p1-execution-control-canary",
            revision="1",
            sample_digest=sample.sha256,
            operator_id="operator:local",
            authorization_basis=(
                "Ordivon-maintained execution-control canary compiled locally to prove the "
                "DaVinci staging tree remains writable while PE execution is denied at both "
                "the root and a nested descendant inside a disposable no-network Windows KVM Guest."
            ),
        ...
        spec = EvaluationSpec(
            evaluation_id="evaluation:windows-kvm-p1-execution-control-canary",
            revision="1",
            sample=sample,
            authority=authority,
