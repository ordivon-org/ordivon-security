...
_FIXTURE_ID = "ordivon-p1-observer-runtime-probe-v1"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Execute a maintained diagnostic wrapper around the sealed P1 Observer using the "
            "same bounded roots and collection limits as the production orchestrator."
        )
    )
...
def _write_header(script: bytes, path: Path) -> None:
    ...
    path.write_text(
        "#ifndef ORDIVON_P1_OBSERVER_RUNTIME_SCRIPT_H\n"
        "#define ORDIVON_P1_OBSERVER_RUNTIME_SCRIPT_H\n"
...
def _compile_observer_runtime_probe(output_path: Path) -> JsonObject:
    launcher = _resource("p1_observer_runtime_launcher.c")
    script = _resource("p1_observer_runtime_probe.ps1")
    ...
    for path in (launcher, script, compiler, objdump):
        if path.is_symlink() or not path.is_file():
            raise ValueError(f"P1 Observer runtime probe dependency is missing or unsafe: {path}")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.parent.chmod(0o700)
    if output_path.exists():
        raise FileExistsError(f"P1 Observer runtime probe output already exists: {output_path}")
    with tempfile.TemporaryDirectory(prefix="ordivon-p1-observer-probe-") as temp_name:
        temp = Path(temp_name)
        header = temp / "p1_observer_runtime_script.h"
    ...
    if matches:
        ...
        raise ValueError(f"P1 Observer runtime probe imports prohibited network APIs: {matches}")
    ...
    return {
        ...
        "wrapperScriptDigest": _digest(script),
...
def main() -> None:
    ...
    compilation = _compile_observer_runtime_probe(fixture_path)
    try:
        ...
        observation = ObservationPlan(
            plan_id="observation-plan:windows-kvm-p1-observer-runtime-probe",
            revision="1",
        ...
        environment = EnvironmentIdentity(
            environment_id="environment:windows-kvm-p1-observer-runtime-probe",
        ...
        authority = AuthorityManifest(
            authority_id="authority:ordivon-p1-observer-runtime-probe",
            revision="1",
            sample_digest=sample.sha256,
            operator_id="operator:local",
            authorization_basis=(
                "Ordivon-maintained diagnostic wrapper that invokes only the exact sealed P1 "
                "Observer with bounded roots and preserves its runtime error as evidence."
            ),
        ...
        spec = EvaluationSpec(
            evaluation_id="evaluation:windows-kvm-p1-observer-runtime-probe",
            revision="1",
            sample=sample,
            authority=authority,
