...
- **Open research:** AF2 provides the reusable zero-or-more Range-intent surface. AF3 separates communicated claim from independent truth. AE0–AE2 establish information acquisition, costly waiting, and explicit adjudication under `UNKNOWN`. AE3 exposes unavoidable regret and an invented prior when adjudication disappears. AE3-B falsifies raw-history sufficiency; AE3-C shows exact reconstructable reduction stabilizes the tested consumer. EC0 externalizes the reduction and shows the cross-domain invariant is exact computation identity rather than a shared `EvidenceReducer`. EC1 then physically advances one authoritative source across initial, metadata-only, and semantic revisions while retaining the exact old EC0 projection: metadata/generation change alone leaves it applicable; an exact history-dependency change makes the still-integrity-valid projection `STALE_NOT_APPLICABLE`; unavailable current authority remains `UNKNOWN`; a fresh Workspace reproduces all classifications. Thus this consumer needs exact dependency applicability, not TTL, wall-clock freshness, Trust/Reputation, or a generic Freshness service. The AE/EC infrastructure line is sufficiently reduced. AC0 then returns to autonomous multi-Agent behavior: exact actor-specific message projection works without a mailbox or RangeEvent visibility ontology, but the corrected one-shot cheap-talk run is falsified when B activates in the mismatch world and incurs regret 10 despite receiving A's truthful signal. AC1 then freezes the exact AC0 truthful A message and adds only public common knowledge that both Actors optimize the same payoff. B now replies in both worlds, but still activates in the mismatch world for `-10` / regret `10` and invents an unsupplied `60%` activation claim. Thus bidirectional communication mechanics work while aligned incentives still do not supply strategic credibility. AC2 then adds a separately authoritative selective disclosure of A's actual private signal while keeping cheap-talk unverified. B's final reasoning now correctly distinguishes match from mismatch, so the AC0/AC1 epistemic ambiguity is removed; however, the Tool-authoritative pending intent still activates in the mismatch world. AC2 forces pre-admission intent revision and shows that replaceability alone is not enough when the Agent fails to emit the corrective Tool call. IF0 then falsifies explicit Tool finalization as a sufficient cognition-convergence mechanism, and IF1 falsifies exact pending-intent readback plus digest-bound finalization: in both runs the Agent commits activation before its later conclusion correctly prefers hold. IF2 changes one variable by giving the same logical Actor a no-effect-authority deliberation turn first; that deliberation correctly derives an empty candidate effect set, and the later authority phase submits, reviews and finalizes `[]`. This is accepted only for the exact AC2 consumer and does not yet force a universal finalization/readback protocol. IF3 performs that subtraction: it retains the no-effect deliberation phase, removes IF1 readback/finalization entirely, and returns to ordinary AF2 `submit_range_intents`. The Agent first requests only a coordination message, then uses ordinary pre-admission revision to replace it with `[]`; final activation remains absent. This accepts deliberation-before-authority with ordinary AF2 in the exact consumer and rejects mandatory IF1 ceremony there. The next pressure should move to an independent consumer or Harness-native experiment rather than adding another Security confirmation layer. [`docs/AGENT-FIRST-INTENT-AF2.md`](docs/AGENT-FIRST-INTENT-AF2.md), [`docs/AGENT-FIRST-DECEPTION-AF3.md`](docs/AGENT-FIRST-DECEPTION-AF3.md), [`docs/ADVERSARIAL-EPISTEMICS-AE0.md`](docs/ADVERSARIAL-EPISTEMICS-AE0.md), [`docs/ADVERSARIAL-EPISTEMICS-AE1.md`](docs/ADVERSARIAL-EPISTEMICS-AE1.md), [`docs/ADVERSARIAL-EPISTEMICS-AE2.md`](docs/ADVERSARIAL-EPISTEMICS-AE2.md), [`docs/ADVERSARIAL-EPISTEMICS-AE3.md`](docs/ADVERSARIAL-EPISTEMICS-AE3.md), [`docs/ADVERSARIAL-EPISTEMICS-AE3B.md`](docs/ADVERSARIAL-EPISTEMICS-AE3B.md), [`docs/ADVERSARIAL-EPISTEMICS-AE3C.md`](docs/ADVERSARIAL-EPISTEMICS-AE3C.md), [`docs/EVIDENCE-COMPUTATION-EC0.md`](docs/EVIDENCE-COMPUTATION-EC0.md), [`docs/EVIDENCE-FRESHNESS-EC1.md`](docs/EVIDENCE-FRESHNESS-EC1.md), [`docs/AUTONOMOUS-COMMUNICATION-AC0.md`](docs/AUTONOMOUS-COMMUNICATION-AC0.md), [`docs/INCENTIVE-COMMUNICATION-AC1.md`](docs/INCENTIVE-COMMUNICATION-AC1.md), [`docs/VERIFIABLE-DISCLOSURE-AC2.md`](docs/VERIFIABLE-DISCLOSURE-AC2.md), [`docs/INTENT-CONVERGENCE-IF0-IF2.md`](docs/INTENT-CONVERGENCE-IF0-IF2.md), and [`docs/INTENT-CEREMONY-ABLATION-IF3.md`](docs/INTENT-CEREMONY-ABLATION-IF3.md) define the current boundary.
...
uv run ordivon-security-windows-kvm-p1-materialize-execution-media \
  --contract research/cases/windows-kvm-p1-davinci-case-a-execution.json \
  --case-manifest research/cases/windows-kvm-p1-davinci-case-a-original-repack.json \
  --transform-manifest research/cases/windows-kvm-p1-davinci-case-a-transform.json \
