...
- **Open research:** AF2 provides a physically consumed zero-or-more Range-intent surface without Contest ticks. AF3 separates communicated claim from complete independent truth. AE0 shows `UNKNOWN` can trigger explicit information acquisition; AE1 shows it can persist through costly waiting. AE2 adds two independently sourced sensor observations that disagree while neither is world truth: the shared Defender policy refuses arbitrary source selection, requests explicit adjudication, and only authoritative current world truth separates healthy hold from compromised quarantine. Trust/Reputation, source-history, confidence and freshness remain unforced. The next pressure is conflict when adjudication is unavailable/expensive or repeated source history becomes causally useful; C1-O stays parked until a stale-but-integrity-valid consumer appears. [`docs/AGENT-FIRST-INTENT-AF2.md`](docs/AGENT-FIRST-INTENT-AF2.md), [`docs/AGENT-FIRST-DECEPTION-AF3.md`](docs/AGENT-FIRST-DECEPTION-AF3.md), [`docs/ADVERSARIAL-EPISTEMICS-AE0.md`](docs/ADVERSARIAL-EPISTEMICS-AE0.md), [`docs/ADVERSARIAL-EPISTEMICS-AE1.md`](docs/ADVERSARIAL-EPISTEMICS-AE1.md), and [`docs/ADVERSARIAL-EPISTEMICS-AE2.md`](docs/ADVERSARIAL-EPISTEMICS-AE2.md) define the accepted boundary.
