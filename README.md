...
- an accepted C1-D fresh-controller continuation profile where the Windows Guest itself consumes peer A, the original controller dies during peer-B materialization, and a fresh process continues from durable effect/resource identity plus Host placement to peer-b-present without restoring the old Range object/event stream or first closing the world; the run exposes durable successor ownership as the next unresolved race;
