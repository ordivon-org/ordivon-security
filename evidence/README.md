...
The CA6 tactical-adaptation acceptance index is retained at [`acceptance/ca6-tactical-adaptation-c59aa0a.json`](acceptance/ca6-tactical-adaptation-c59aa0a.json). It binds committed apparatus revision `c59aa0a`, four held-out two-turn semantic worlds grounded in verified CA1-CA4 capability properties, a fixed scripted baseline, a property-driven deterministic adaptive baseline, and a real `deepseek-v4-flash`/Harness treatment using accepted deliberation-before-authority plus ordinary AF2 intent. Static fails three worlds; both adaptive treatments succeed all four and have the same canonical oracle-regret vector. It also retains direct-AF2 budget failure and non-canonical stochastic model evidence. It authorizes no real exploit execution, credential movement, malware, external action, Campaign, Organization or shared provider gateway.

The CA7 strategic-admission acceptance index is retained at [`acceptance/ca7-campaign-organization-gate-0ba1ec7.json`](acceptance/ca7-campaign-organization-gate-0ba1ec7.json). It binds the accepted CA6 tactical result, a current-source audit showing no active CampaignState/Organization/PersistentOpponentModel/Coevolution implementation, and the negative admission decision that none of those subsystems nor a cross-fidelity strategic law is currently earned. It records explicit causal reopen conditions instead of treating roadmap chronology as implementation authority.

The Research Corpus P0 acceptance index is retained at [`acceptance/research-corpus-p0-9fa5a7f.json`](acceptance/research-corpus-p0-9fa5a7f.json). It binds committed implementation revision `9fa5a7f`, a clean-source 399-test/E9-F Runtime acceptance, exact CA2/EICAR/目标产品B seed manifests, private SampleVault local-import dogfood, all-head plus historical-revision verification, claim truth-role separation, exact SHA-256 Sample identity, metadata-only 目标产品B retention, and denied-by-default Sample execution. It authorizes no malware download/upload, provider mirror/synchronization, public sample distribution, third-party execution or provider classification promotion to independent Security truth.


The Research Corpus K1 currentness acceptance index is retained at [`acceptance/research-corpus-k1-currentness-e64926a.json`](acceptance/research-corpus-k1-currentness-e64926a.json). It accepts read-only candidate-vs-head provider snapshot comparison after a controlled stale/current falsifier and rejects background polling, mirroring, global TTL freshness, and automatic head mutation.
...
The later 目标产品B closed-history physical series is now retained in current main rather than only in the closed detached ref. [`acceptance/windows-kvm-p1-case-a-causal-edge-d274e7b.json`](acceptance/windows-kvm-p1-case-a-causal-edge-d274e7b.json) accepts the exact wrapper→exact contained main/malicious-MSI edge; [`acceptance/windows-kvm-p1-case-a-post-edge-d1-outer-role-b89afeb.json`](acceptance/windows-kvm-p1-case-a-post-edge-d1-outer-role-b89afeb.json) resolves the outer 厂商B MSI as a separate wrapper-spawned prerequisite; D2/D3 retain the bootstrap-context falsifier and malicious-PowerShell edge; and [`acceptance/windows-kvm-p1-case-a-d4-bounded-no-secondary-effects-197053c.json`](acceptance/windows-kvm-p1-case-a-d4-bounded-no-secondary-effects-197053c.json) retains the bounded zero-NIC absence of downstream bitsadmin/staging/task effects. [`acceptance/windows-kvm-p1-caseb-research-closeout-f8127d6.json`](acceptance/windows-kvm-p1-caseb-research-closeout-f8127d6.json) binds the closed-cycle interpretation and explicitly forbids automatic sample reexecution, network enablement, or promotion of the unresolved loader-causality question. These later records supersede only the old *reachability unknown* state; they do not reverse the product rejection or establish standing execution authority.
...
The post-CA O1 carrier-observability acceptance index is retained at [`acceptance/post-ca-o1-carrier-observability.json`](acceptance/post-ca-o1-carrier-observability.json). It accepts a scoped same-effect MSI telemetry contrast, preserves invalid PowerShell/retry trials as non-conclusions, and does not admit Sysmon/EDR/SIEM or a universal stealth ranking.

The post-CA P1 physical-adaptation acceptance index is retained at [`acceptance/post-ca-p1-physical-adaptation.json`](acceptance/post-ca-p1-physical-adaptation.json). It binds the owned local physical provider world, deterministic adaptive 4/4 result, clean Harness/model treatment, receipt-versus-fresh-truth separation and residual closure, while rejecting any CA5 Gateway/Campaign/Organization promotion.
- `acceptance/ordinary-security-consumption-r1-cac3fe1.json` — clean-source ordinary Agent routing, ResearchCorpus memory and bounded Blue incident acceptance for implementation `cac3fe1`; records negative EDR/SIEM and higher-fidelity gates plus the non-attribution boundary for post-route Harness/model friction.
